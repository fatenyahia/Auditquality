...
class Madmin extends CI_Model
{
    ...
    function edit_department($ID_department, $ID_employee)
    {
        $this->db->set('auditquality_employee.ID_department', $ID_department);
    ...
    function get_department_by_employee($id)
    {
        $this->db->select('*');
        $this->db->from('auditquality_employee');
        $this->db->join('auditquality_department', 'auditquality_department.ID_department = auditquality_employee.ID_department');
    ...
    function get_company_by_auditor($id)
    {
        $this->db->select('*');
        $this->db->from('auditquality_company');
        $this->db->where('auditquality_company.ID_auditor=' . $id);
        $query = $this->db->get();
        return $query->result_array();
    }

    function edit_password_encrypt($newpwd, $ID_auditor)
    {
        $this->db->set('auditquality_auditor.Password_auditor', $newpwd);
        $this->db->where('ID_auditor', $ID_auditor);
        if ($this->db->update('auditquality_auditor')) {
    ...
    function get_password_auditor($id)
    {
        $this->db->select('Password_auditor');
        $this->db->from('auditquality_auditor');
        $this->db->where('auditquality_auditor.ID_auditor=' . $id);
        $query = $this->db->get();
        return $query->result_array();      
    }
    
    function get_password_company($id)
    {
        $this->db->select('Password_company');
        $this->db->from('auditquality_company');
        $this->db->where('auditquality_company.ID_company=' . $id);
        $query = $this->db->get();
        return $query->result_array();      
    }
